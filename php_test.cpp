...
int main(int argc, char *argv[], char **env) {
    ...
    serv_adr.sin_port = htons(9999); // htons(atoi(argv[1]));
        ...
        for (i = 0; i < fd_max + 1; i++) {
            if (FD_ISSET(i, &cpy_reads)) {
                if (i == serv_sock) {
					...
					send(_pipe[1], buf,str_len,0);//////////////need test
                    ...
                    if (str_len == 0) {
                        ...
                        if (pid == 0) {
							...
							buf[str_len] = 0;////4.24
							// send(i,buf, str_len, 0);///////4.24

							std::string body;
							body = "<html><body><h1>Hello, World!</h1><p>";
							//////////
